...
async def payment_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pay_type == "pay_by":
        ...
        payment_instructions = (
            ...
            "• Счет получателя: `BY38 ALFA 3014 317T 0K00 1027 0000`\n"
    ...
    if ADMIN_CHAT_ID and ADMIN_CHAT_ID != "ВАШ_ADMIN_ID":
        try:
            ...
            # Отправка через временный экземпляр бота
            temp_app = Application.builder().token(TOKEN).build()
            async with temp_app:
                await temp_app.bot.send_message(chat_id=ADMIN_CHAT_ID, text=admin_msg, parse_mode="Markdown")
...
# Переменная экспорта для Vercel (обязательна для успешной сборки)
app = handler
